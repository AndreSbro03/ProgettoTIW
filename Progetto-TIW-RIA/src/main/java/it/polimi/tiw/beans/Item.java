...
public class Item {
	...
	public static Float isValid(String name, String descr, String price) throws Exception {
		/// Some is null
		if (name == null || descr == null || price == null)
			throw new Exception("Fields can't be null");
		if (name.length() < 4 || name.length() >= 64)
			throw new Exception("Name format not valid");
		if (descr.length() < 0 || descr.length() >= 256)
			throw new Exception("Description format not valid");
